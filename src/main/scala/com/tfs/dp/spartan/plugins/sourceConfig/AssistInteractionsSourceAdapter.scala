...
class AssistInteractionsSourceAdapter extends SourceAdapterPlugin {
  ...
  override def readData(spark: SparkSession, finalPaths:Seq[String],
    ...
    val interactionJsons: RDD[String] = encryptedInteractions.mapPartitions { interactionsIter =>
      ...
      val decryptedIter = interactionsIter.map { x => encryptor.decrypt(x) }
      decryptedIter
    }
    val defaultDataDf = spark.read.textFile(schemaLocation+"/schema_AssistInteractions.json")
    val schemaRdd = defaultDataDf.rdd
    spark.sqlContext.read.json(interactionJsons ++ schemaRdd)
  }
}
